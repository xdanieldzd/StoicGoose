...
namespace StoicGoose.Emulation.Display
{
	public class DisplayControllerSphinx : IComponent
	{
		...
		/* REG_BACK_COLOR */
		byte backColorIndex, backColorPalette;
		/* REG_LINE_xxx */
		...
		/* REG_LCD_xxx */
		bool lcdActive, lcdContrastHigh;
		...
		/* REG_DISP_MODE */
		bool displayPackedFormatSet, displayColorFlagSet, display4bppFlagSet;
		/* REG_xTMR_xxx */
		...
		bool isPackedMode => displayPackedFormatSet;
		bool isPlanarMode => !displayPackedFormatSet;
		...
		private void ResetRegisters()
		{
			...
			backColorIndex = backColorPalette = 0;
			...
			displayPackedFormatSet = displayColorFlagSet = display4bppFlagSet = false;
			hBlankTimer.Reset();
			vBlankTimer.Reset();
		}
		...
		private void RenderSCR1(int y, int x)
		{
			...
			if (isColorMode)
			{
				var tileBank = (attribs >> 13) & 0b1;
				tileNum |= (ushort)(tileBank << 9);
				...
				if (IsColorOpaque((byte)tilePal, color))
				{
					...
					WriteToFramebuffer(y, x, (byte)tilePal, color);
