...
namespace StoicGoose.WPF
{
	...
	public class GraphicsHandler
	{
		...
		private static RgbaFile GetEmbeddedSystemIcon(string name) => Resources.GetEmbeddedRgbaFile($"Assets.Icons.{name}");
		private static string GetEmbeddedShaderFile(string name) => Resources.GetEmbeddedText($"Assets.Shaders.{name}");

		private List<string> EnumerateShaders()
		{
			var shaderNames = new List<string>();

			if (!string.IsNullOrEmpty(GetEmbeddedShaderFile($"{DefaultShaderName}.{DefaultManifestFilename}")))
				shaderNames.Add(DefaultShaderName);

			foreach (var file in new DirectoryInfo(App.ShaderPath).EnumerateFiles("*.json", SearchOption.AllDirectories))
		...
		private void ParseSystemIcons()
		{
			...
			{
				var texture = new Texture(GetEmbeddedSystemIcon(filename));
		...
		{
			commonVertexShaderSource = GetEmbeddedShaderFile("Vertex.glsl");
			commonFragmentShaderBaseSource = GetEmbeddedShaderFile("FragmentBase.glsl");
		}
		...
		private (BundleManifest, ShaderProgram) LoadShaderBundle(string name)
		{
			...
			if (!string.IsNullOrEmpty(manifestJson = GetEmbeddedShaderFile($"{name}.{DefaultManifestFilename}")))
				fragmentSource = GetEmbeddedShaderFile($"{name}.{DefaultSourceFilename}");
			else
			{
				var externalManifestFile = Path.Combine(App.ShaderPath, name, DefaultManifestFilename);
				var externalSourceFile = Path.Combine(App.ShaderPath, name, DefaultSourceFilename);
			...
			var glslVersionString = $"#version {App.RequiredGLVersion.Major}{App.RequiredGLVersion.Minor}{App.RequiredGLVersion.Build}";
		...
		public void DrawFrame()
		{
			...
			displayBrightness.Value = 0.0f;//Program.Configuration.Video.Brightness * 0.01f;
			displayBrightness.SubmitToProgram(commonShaderProgram);
			displayContrast.Value = 1.0f;//Program.Configuration.Video.Contrast * 0.01f;
			displayContrast.SubmitToProgram(commonShaderProgram);
			displaySaturation.Value = 1.0f;//Program.Configuration.Video.Saturation * 0.01f;
