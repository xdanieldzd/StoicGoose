...
namespace StoicGoose.GLWindow.Interface.Windows
{
	public class DisassemblerWindow : WindowBase, IDisposable
	{
		...
		protected override void DrawWindow(object userData)
		{
			...
			if (ImGui.Begin(WindowTitle, ref isWindowOpen, ImGuiWindowFlags.NoResize))
			{
				...
				if (ImGui.BeginChild("##processor", NumericsVector2.Zero))
				{
					...
					DrawFlag(drawListProcessor, pos, machine.Cpu.PSW.Carry, "CF"); pos.X += glyphSize.X * 2.5f;
					DrawFlag(drawListProcessor, pos, machine.Cpu.PSW.Parity, "PF"); pos.X += glyphSize.X * 2.5f;
					DrawFlag(drawListProcessor, pos, machine.Cpu.PSW.AuxiliaryCarry, "AF"); pos.X += glyphSize.X * 2.5f;
					DrawFlag(drawListProcessor, pos, machine.Cpu.PSW.Zero, "ZF"); pos.X += glyphSize.X * 2.5f;
					DrawFlag(drawListProcessor, pos, machine.Cpu.PSW.Sign, "SF"); pos.X += glyphSize.X * 2.5f;
					DrawFlag(drawListProcessor, pos, machine.Cpu.PSW.Break, "TF"); pos.X += glyphSize.X * 2.5f;
					DrawFlag(drawListProcessor, pos, machine.Cpu.PSW.InterruptEnable, "IF"); pos.X += glyphSize.X * 2.5f;
					DrawFlag(drawListProcessor, pos, machine.Cpu.PSW.Direction, "DF"); pos.X += glyphSize.X * 2.5f;
					DrawFlag(drawListProcessor, pos, machine.Cpu.PSW.Overflow, "OF"); pos.X += glyphSize.X * 2.5f;
					pos.X = posStart.X;
					pos.Y += height * 1.5f;
		...
		private void DrawFlag(ImDrawListPtr drawList, NumericsVector2 position, bool flag, string label)
		{
			drawList.AddText(position, flag ? colorText : colorDisabled, label);
		}
