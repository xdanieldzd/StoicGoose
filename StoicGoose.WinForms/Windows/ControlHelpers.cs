...
using StoicGoose.WinForms.Windows.Controls;
...
namespace StoicGoose.WinForms.Windows
{
    public static class ControlHelpers
    {
        ...
        public class SettingsPage
        {
            ...
            readonly Dictionary<string, Control> controlList = [];
            readonly List<SettingsPage> subPages = [];
        ...
        public static Control[] CreateKeyInput(object obj, string propertyName, string keyName)
        {
            ...
            var textBoxEx = new TextBoxEx()
            {
                ...
                TabIndex = Array.IndexOf([.. dict.Keys], keyName),
                Tag = dict[keyName],
                TimerInterval = 100
            };
            textBoxEx.TimerTick += (s, e) =>
            {
                if (s is TextBoxEx textBoxEx && textBoxEx.Tag is List<string> keyValues)
                {
                    ...
                    for (var i = 0; i < ControllerManager.MaxControllers; i++)
                    {
                        ...
                        {
                            keyValues = [.. keyValues.Append($"{InputHandler.GamepadPrefix}{i}+{controller.Buttons}").OrderBy(x => x).Distinct()];
                            doUpdate = true;
                        }
                        if (controller.LeftThumbstick != ThumbstickPosition.Zero)
                        {
                            if (controller.LeftThumbstick.X < -0.5f) keyValues = [.. keyValues.Append($"{InputHandler.GamepadPrefix}{i}+{nameof(controller.LeftThumbstick)}Left").OrderBy(x => x).Distinct()];
                            else if (controller.LeftThumbstick.X > 0.5f) keyValues = [.. keyValues.Append($"{InputHandler.GamepadPrefix}{i}+{nameof(controller.LeftThumbstick)}Right").OrderBy(x => x).Distinct()];
                            if (controller.LeftThumbstick.Y < -0.5f) keyValues = [.. keyValues.Append($"{InputHandler.GamepadPrefix}{i}+{nameof(controller.LeftThumbstick)}Down").OrderBy(x => x).Distinct()];
                            else if (controller.LeftThumbstick.Y > 0.5f) keyValues = [.. keyValues.Append($"{InputHandler.GamepadPrefix}{i}+{nameof(controller.LeftThumbstick)}Up").OrderBy(x => x).Distinct()];
                            doUpdate = true;
                        }
                        if (controller.RightThumbstick != ThumbstickPosition.Zero)
                        {
                            if (controller.RightThumbstick.X < -0.5f) keyValues = [.. keyValues.Append($"{InputHandler.GamepadPrefix}{i}+{nameof(controller.RightThumbstick)}Left").OrderBy(x => x).Distinct()];
                            else if (controller.RightThumbstick.X > 0.5f) keyValues = [.. keyValues.Append($"{InputHandler.GamepadPrefix}{i}+{nameof(controller.RightThumbstick)}Right").OrderBy(x => x).Distinct()];
                            if (controller.RightThumbstick.Y < -0.5f) keyValues = [.. keyValues.Append($"{InputHandler.GamepadPrefix}{i}+{nameof(controller.RightThumbstick)}Down").OrderBy(x => x).Distinct()];
                            else if (controller.RightThumbstick.Y > 0.5f) keyValues = [.. keyValues.Append($"{InputHandler.GamepadPrefix}{i}+{nameof(controller.RightThumbstick)}Up").OrderBy(x => x).Distinct()];
                            doUpdate = true;
                        }
                        if (controller.LeftTrigger > 0.5f)
                        {
                            keyValues = [.. keyValues.Append($"{InputHandler.GamepadPrefix}{i}+{nameof(controller.LeftTrigger)}").OrderBy(x => x).Distinct()];
                            doUpdate = true;
                        }
                        if (controller.RightTrigger > 0.5f)
                        {
                            keyValues = [.. keyValues.Append($"{InputHandler.GamepadPrefix}{i}+{nameof(controller.RightTrigger)}").OrderBy(x => x).Distinct()];
                            doUpdate = true;
                        }
            ...
            {
                if (s is TextBoxEx textBoxEx && textBoxEx.Tag is List<string> keyValues && winToOpenTkKeyTranslator.TryGetValue(e.KeyCode, out OTKKeys value))
                {
                    keyValues = [.. keyValues.Append($"{InputHandler.KeyboardPrefix}+{value}").OrderBy(x => x).Distinct()];
                    textBoxEx.Text = string.Join(", ", keyValues.Select(x => $"{x.Split('+')[1]} ({x.Split('+')[0]})"));
                    textBoxEx.Tag = keyValues;
                }
            };
            ...
            return [label, textBoxEx];
        }

        private static string GetDisplayName(Type type, string propertyName)
        {
            return (type.GetProperty(propertyName).GetCustomAttribute<DisplayNameAttribute>(false))?.DisplayName ?? propertyName;
        }

        private static string GetDescription(Type type, string propertyName)
        {
            return (type.GetProperty(propertyName).GetCustomAttribute<DescriptionAttribute>(false))?.Description ?? $"{propertyName} has no description";
        }

        private static (T, T) GetRange<T>(Type type, string propertyName)
        {
            if (type.GetProperty(propertyName).GetCustomAttribute<RangeAttribute>(false) is not RangeAttribute rangeAttrib || rangeAttrib.OperandType != typeof(T)) return (default, default);
