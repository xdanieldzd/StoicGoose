...
namespace StoicGoose.WinForms.Handlers
{
    public sealed class DatabaseHandler
    {
        ...
        {
            return datFiles.Select(x => x.Value.Game).Select(x => x.FirstOrDefault(x => x.Rom.Any(y => y.Crc.Equals($"{romCrc32:x8}", StringComparison.InvariantCultureIgnoreCase) && y.Size.Equals($"{romSize:D}", StringComparison.InvariantCultureIgnoreCase)))).FirstOrDefault(x => x != null);
        }
