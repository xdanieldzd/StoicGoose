...
namespace StoicGoose.Core.Processor
{
	...
	{
		readonly Action[] instructions = new Action[256];
		...
		private void GenerateInstructionHandlers()
		{
			...
			instructions[0x24] = () => { Wait(1); aw.Low = AND(aw.Low, Fetch8()); };
			instructions[0x25] = () => { Wait(1); aw.Word = AND(aw.Word, Fetch16()); };
			instructions[0x26] = () => { /* Prefix Segment DS1 */ };
			...
			instructions[0x2E] = () => { /* Prefix Segment PS */ };
			...
			instructions[0x36] = () => { /* Prefix Segment SS */ };
			...
			instructions[0x3E] = () => { /* Prefix Segment DS0 */ };
			...
			instructions[0x69] = () => { Wait(5); modRM = Fetch8(); SetRegister16((UInt16)MUL(GetMemory16(), Fetch16())); };
			instructions[0x6A] = () => { PUSH((UInt16)(SByte)Fetch8()); };
			instructions[0x6B] = () => { Wait(5); modRM = Fetch8(); SetRegister16((UInt16)MUL(GetMemory16(), (UInt16)(SByte)Fetch8())); };
			...
			instructions[0x80] = () => { GroupImmediate8(); };
			instructions[0x81] = () => { GroupImmediate16(); };
			instructions[0x82] = () => { GroupImmediate8(); };
			instructions[0x83] = () => { GroupImmediateSign(); };
			...
			instructions[0x99] = () => { Wait(1); dw.Word = (UInt16)((aw.Word & 0x8000) == 0x8000 ? 0xFFFF : 0x0000); };
			instructions[0x9A] = () => { Wait(9); var pc = Fetch16(); var ps = Fetch16(); PUSH(this.ps); PUSH(this.pc); this.pc = pc; this.ps = ps; };
			instructions[0x9B] = () => { Wait(1); };
			instructions[0x9C] = () => { Wait(1); PUSH(psw.Value); };
			instructions[0x9D] = () => { Wait(2); psw.Value = POP(); };
			instructions[0x9E] = () => { Wait(4); psw.Value = (UInt16)((psw.Value & 0xFF00) | aw.High); };
			...
			instructions[0xA6] = () => { Wait(4); CMPBK8(); };
			instructions[0xA7] = () => { Wait(4); CMPBK16(); };
			instructions[0xA8] = () => { Wait(1); AND(aw.Low, Fetch8()); };
			instructions[0xA9] = () => { Wait(1); AND(aw.Word, Fetch16()); };
			instructions[0xAA] = () => { Wait(2); STM8(); };
			instructions[0xAB] = () => { Wait(2); STM16(); };
			instructions[0xAC] = () => { Wait(2); LDM8(); };
			instructions[0xAD] = () => { Wait(2); LDM16(); };
			instructions[0xAE] = () => { Wait(3); CMPM8(); };
			instructions[0xAF] = () => { Wait(3); CMPM16(); };
			...
			instructions[0xC0] = () => { Wait(3); GroupShift8(0, true); };
			instructions[0xC1] = () => { Wait(3); GroupShift16(0, true); };
			instructions[0xC2] = () => { Wait(5); var offset = Fetch16(); pc = POP(); sp += offset; };
			instructions[0xC3] = () => { Wait(5); pc = POP(); };
			instructions[0xC4] = () => { Wait(4); modRM = Fetch8(); SetRegister16(GetMemory16(0)); ds1 = GetMemory16(2); };
			instructions[0xC5] = () => { Wait(4); modRM = Fetch8(); SetRegister16(GetMemory16(0)); ds0 = GetMemory16(2); };
			...
			instructions[0xCA] = () => { Wait(8); var offset = Fetch16(); pc = POP(); ps = POP(); sp += offset; };
			instructions[0xCB] = () => { Wait(7); pc = POP(); ps = POP(); };
			...
			instructions[0xCF] = () => { Wait(9); pc = POP(); ps = POP(); psw.Value = POP(); };
			instructions[0xD0] = () => { Wait(1); GroupShift8(1, false); };
			instructions[0xD1] = () => { Wait(1); GroupShift16(1, false); };
			instructions[0xD2] = () => { Wait(3); GroupShift8(cw.Low, false); };
			instructions[0xD3] = () => { Wait(3); GroupShift16(cw.Low, false); };
			...
			instructions[0xD7] = () => { Wait(4); aw.Low = ReadMemory8(SegmentViaPrefix(ds0), (UInt16)(bw.Word + aw.Low)); };
			...
			instructions[0xDE] = () => { /* '' */ };
			instructions[0xDF] = () => { /* '' */ };
			instructions[0xE0] = () => { DBNZNE(); };
			instructions[0xE1] = () => { DBNZE(); };
			instructions[0xE2] = () => { DBNZ(); };
			instructions[0xE3] = () => { BranchIf(cw.Word == 0); };
			...
			instructions[0xE8] = () => { Wait(4); var offset = (Int16)Fetch16(); PUSH(pc); pc = (UInt16)(pc + offset); };
			instructions[0xE9] = () => { Wait(3); var offset = (Int16)Fetch16(); pc = (UInt16)(pc + offset); };
			instructions[0xEA] = () => { Wait(6); var pc = Fetch16(); var ps = Fetch16(); this.ps = ps; this.pc = pc; };
			instructions[0xEB] = () => { Wait(3); var offset = (SByte)Fetch8(); pc = (UInt16)(pc + offset); };
			...
			instructions[0xF0] = () => { /* Prefix BUSLOCK */ };
			instructions[0xF1] = () => { /* invalid */ };
			instructions[0xF2] = () => { /* Prefix REPNE/REPNZ */ };
			instructions[0xF3] = () => { /* Prefix REP/REPE/REPZ */ };
			instructions[0xF4] = () => { Wait(8); isHalted = true; };
			instructions[0xF5] = () => { Wait(4); psw.Carry = !psw.Carry; };
			instructions[0xF6] = () => { Group1Misc8(); };
			instructions[0xF7] = () => { Group1Misc16(); };
			...
			instructions[0xFE] = () => { Group2Misc8(); };
			instructions[0xFF] = () => { Group2Misc16(); };
		}
	}
}
